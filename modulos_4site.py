...
GASTO_POR_TIPO = {
    # pct_gasto recalibrado: el valor original asumía que TODA la población visita
    # con frecuencia de cliente habitual (8-10 visitas/mes) — sobreestimaba 2x.
    # Ajustado contra observación real de campo (Toluca/Metepec, jun 2026).
    "cafe_premium":       {"pct_gasto": 0.02, "ticket_promedio": 180, "visitas_mes": 8},
    "cafe_casual":        {"pct_gasto": 0.0175,"ticket_promedio": 120, "visitas_mes": 10},
...
def calcular_mercado_potencial(datos_inegi, tipo_negocio_key, num_competidores, competidores=None, capacidad_asientos=None):
    """
    Calcula el tamaño de mercado potencial en el área de 500m.
    Metodología: población × ingreso × % gasto × factor de captura,
    ajustado por la calidad de la competencia existente (rating) y,
    si se conoce, acotado por la capacidad física del local (asientos).
    """
    ...
    # Zona SIN competencia no es automáticamente la mejor: puede significar
    # demanda no comprobada para la categoría en esa zona — más incertidumbre,
    # depende casi todo de tu estrategia digital o de tráfico de paso casual.
    # Competencia moderada, en cambio, valida que ahí SÍ se busca este tipo de
    # negocio (tráfico de categoría) — parte de ese flujo también te llega a ti.
    zona_nueva_sin_validar = num_competidores == 0
    if num_competidores == 0:
        factor_captura = 0.08   # Demanda sin comprobar: mayor incertidumbre, no el mejor caso
    elif num_competidores <= 2:
        factor_captura = 0.09   # Poca competencia + algo de demanda ya validada
    elif num_competidores <= 5:
        factor_captura = 0.08   # Demanda validada (tráfico de categoría), competencia manejable
    elif num_competidores <= 10:
        factor_captura = 0.06
    else:
        factor_captura = 0.04   # Mercado saturado, pero con tráfico de categoría comprobado

    # Ajuste por calidad de la competencia: si la mayoría de los
    # competidores ya tiene rating excelente (≥4.3★), el mercado está
    # fidelizado y es más difícil capturar clientes nuevos — reduce el
    # factor de captura en vez de solo contar cuántos competidores hay.
    rating_promedio = 0.0
    pct_excelente   = 0.0
    if competidores:
        ...
        if ratings:
            ...
            pct_excelente   = sum(1 for r in ratings if r >= 4.3) / len(ratings)

    calidad_penalty = 1.0 - (pct_excelente * 0.25) - max(0.0, rating_promedio - 4.0) * 0.08
    calidad_penalty = max(0.5, min(1.0, calidad_penalty))
    factor_captura  *= calidad_penalty
    ...
    es_nicho = gasto_info.get("pct_gasto", 0) < 0.015
    if es_nicho:
        ...
        # También se castiga por calidad de competencia, igual que el caso masivo
        clientes_dia_estimados = max(1, int(poblacion * 0.107 * calidad_penalty / 30))
    else:
        clientes_dia_estimados = max(1, int(mercado_captura_mensual / ticket / 30))  # 30 días

    # Tope por capacidad física: un local con pocos asientos no puede sostener
    # más clientes/día que su capacidad real de atención, sin importar qué
    # tan grande sea el mercado teórico. Asumimos ~1.3 vueltas/asiento/día
    # para un negocio de consumo en sitio (café, restaurante) — conservador,
    # ya que la mayoría de las visitas no llenan el local todo el día.
    capacidad_limitada = False
    if capacidad_asientos:
        tope_por_capacidad = int(capacidad_asientos * 1.3)
        if clientes_dia_estimados > tope_por_capacidad:
            clientes_dia_estimados = tope_por_capacidad
            capacidad_limitada = True
    ...
    return {
        ...
        "calidad_penalty_pct":        round(calidad_penalty * 100, 0),
        "competidores_rating_promedio": round(rating_promedio, 1),
        "competidores_pct_excelente":   round(pct_excelente * 100, 0),
        "zona_sin_competencia_validada": zona_nueva_sin_validar,
        ...
        "metodologia": (
            ...
            f"{factor_captura*100:.1f}% captura estimada "
            f"(ajustada {calidad_penalty*100:.0f}% por calidad de competencia: "
            f"{pct_excelente*100:.0f}% excelente, rating prom. {rating_promedio:.1f}★)"
            + (
                " · Zona sin competidores: demanda de esta categoría aún no comprobada en la zona, "
                "mayor incertidumbre — el resultado depende más de tu estrategia digital o tráfico de paso."
                if zona_nueva_sin_validar else
                " · Hay competencia en la zona: esto valida que sí se busca este tipo de negocio aquí "
                "(tráfico de categoría), parte de ese flujo también puede llegarte a ti."
            )
            + (f" · Tope aplicado por capacidad del local (~{capacidad_asientos} asientos)." if capacidad_limitada else "")
        )
    }
...
def generar_forecast(mercado, tipo_negocio_key, score_viabilidad, inversion_min, idioma="es"):
    ...
    # - Nicho: partir del ingreso real (clientes × ticket × días)
