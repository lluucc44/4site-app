...
Paleta exacta:
  #13243B  fondo panel portada
  #2A6FDB  dorado acento
  #1F9D57  verde score excelente
  #D98A1F  amarillo score bueno
  #E0524A  rojo score bajo
  #262626  card score (gris muy oscuro)
  #20364F  footer / header fondo
  #F5F5F5  fondo KPI cards
  #6B6B6B  texto secundario
  #20364F  texto primario (no negro puro)
...
def _build_pptx(payload: dict) -> BytesIO | None:
    ...
    C = {
        ...
        "rojo":    "E0524A",
        "blanco":  "FFFFFF",
        "oscuro":  "20364F",  # texto primario / headers
        "gris_t":  "6B6B6B",  # texto secundario
        "gris_c":  "F5F5F5",  # fondo cards
        "card_sc": "262626",  # card score (gris muy oscuro)
        "footer":  "20364F",  # footer bg
        "panel":   "20364F",  # panel portada (gris oscuro ppttipo)
    }
    ...
    def interp(sl, y, text, h=0.55):
        rect(sl, 0.35, y, 12.6, h, "F0F6FF", C["dorado"], 0.6)
    ...
    if isB or isP or isPr:
        ...
        nc  = {"A":"7B1FA2","A/B":"9C27B0","B":"1976D2","B/C+":"0097A7","C+":"2E7D32",
               "C":"558B2F","C/D+":"F57F17","D+":"E65100","D/E":"B71C1C"}.get(nse,"2A6FDB")

        # 6 KPI cards en una fila
        kpis6 = [(f"{int(pob):,}",  f"Población {yr}",   "2A6FDB"),
                 (f"{int(viv):,}",  "Viviendas",          "2A6FDB"),
                 (nse,              "NSE Predominante",    nc),
                 (f"${int(ing):,}", "Ingreso prom/mes",   "1F9D57"),
                 (f"${int(gst):,}", "Gasto prom/mes",     "D98A1F"),
    ...
    if (isP or isPr) and tkt:
        ...
        kpi(sl, 4.45, 1.0, 3.9, rz, "Nivel de precio del área", "2A6FDB")
        ...
        pm2 = {"premium": (C["dorado"], f"PREMIUM: Cobrar por encima de ${int(tk_c):,} MXN. Tu ubicación y NSE lo justifican. Enfoca comunicación en calidad y experiencia."),
               "competitivo": ("2A6FDB", f"COMPETITIVA: Igualar ${int(tk_c):,} MXN. Gana por conveniencia y servicio. Compensa con programa de fidelización."),
        ...
        pc2, pt2 = pm2.get(pos, ("2A6FDB", f"Calibra tu precio según tu propuesta de valor vs competencia actual (${int(tk_c):,} MXN promedio zona)."))
    ...
    if (isP or isPr) and traf:
        ...
        kpi(sl, 0.3,  0.98, 3.0, traf.get("nivel_general","—"), "Flujo general",   "2A6FDB")
        ...
        if merc:
            ...
            kpi(sl, 0.3,  5.12, 3.0, f"${int(mt):,}",  "Mercado total área/mes",  "2A6FDB")
    ...
    if tk == "free" and comp:
        ...
        kpi(sl, 0.3,  0.98, 4.5, len(comp),     "Competidores cercanos", "2A6FDB")
    ...
    if tk == "free" and dem:
        ...
        _nc_f={"A":"7B1FA2","A/B":"9C27B0","B":"1976D2","B/C+":"0097A7","C+":"2E7D32","C":"558B2F","C/D+":"F57F17","D+":"E65100","D/E":"B71C1C"}.get(_nse_f,"2A6FDB")
        kpi(sl,0.3,0.98,3.0,f"{int(_pob_f):,}","Poblacion area","2A6FDB")
