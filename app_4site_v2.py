...
# ============================================
# SISTEMA DE USOS — máximo 3 análisis por código
# ============================================
# Cada código puede usarse hasta MAX_USOS_POR_CODIGO veces.
# Los usos se cuentan en session_state (misma sesión) y en un
# archivo local codigos_usos.json (persistente entre sesiones).
# En Streamlit Cloud el archivo no persiste entre deploys; si
# necesitas persistencia real, migra el conteo a Google Sheets.

MAX_USOS_POR_CODIGO = 3
...
def _guardar_conteo_usos(conteo: dict):
    ...
    try:
        ...
        pass  # En Streamlit Cloud sin volumen persistente, solo session_state

def usos_restantes(codigo: str) -> int:
    """
    Retorna cuántos usos le quedan al código.
    Combina conteo de archivo + conteo de sesión actual.
    Los códigos -TEST tienen usos ilimitados.
    """
    ...
    usos_archivo = conteo.get(codigo, 0)

    # Usos en la sesión actual (para evitar doble-conteo en la misma sesión)
    usos_sesion = st.session_state.get("usos_sesion_actual", {}).get(codigo, 0)

    total_usado = usos_archivo  # el archivo ya incluye usos previos de otras sesiones
    # En la sesión actual solo contamos lo que aún no está en el archivo
    restantes = MAX_USOS_POR_CODIGO - total_usado
...
def activar_codigo(codigo):
    ...
    # Marcar como 'usado' en Google Sheets — única fuente de verdad real
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .card {{ background: #1A1A1A; border-radius: 12px; padding: 20px 18px; min-width: 195px; max-width: 215px; text-align: left; display: flex; flex-direction: column; justify-content: space-between; }}
    ...
    <div class="card card-premium">
      <div>
        ...
        <ul class="features">
          ...
          <li>ROI estimado y meses para recuperar inversión</li>
          <li>Ticket recomendado para tu negocio específico</li>
          <li>Comparativa entre hasta 3 ubicaciones candidatas</li>
          <li>Análisis de canibalización: ¿este punto afecta otro tuyo?</li>
          <li>Diagnóstico de riesgo por competidor dominante</li>
          <li>Recomendación de horarios de mayor demanda</li>
