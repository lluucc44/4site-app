...
- Día más bajo: {trafico_data.get('dia_bajo','N/A')}"""
    ...
    if mercado_data:
        datos_pro += f"""
MERCADO POTENCIAL:
- Mercado total área/mes: ${_n(mercado_data.get('mercado_total_mensual',0)):,}
- Tu captura estimada/mes: ${_n(mercado_data.get('mercado_captura_mensual',0)):,}
- Tasa de captura: {_n(mercado_data.get('factor_captura_pct',0))}%
- Clientes/día estimados: {_n(mercado_data.get('clientes_dia_estimados',0))}
- Ticket promedio del negocio: ${_n(mercado_data.get('ticket_promedio',0)):,} MXN (usar este dato — no calcular desde otras cifras)
- Ingreso operativo estimado: ${_n(mercado_data.get('ingreso_operativo_mensual',0)):,} MXN/mes ({_n(mercado_data.get('clientes_dia_estimados',0))} clientes × ${_n(mercado_data.get('ticket_promedio',0)):,} ticket × {_n(mercado_data.get('dias_operacion',26))} días) — este es el ingreso real esperado del negocio, diferente al mercado potencial total"""

    if forecast_data:
        esc = forecast_data.get('escenarios', {})
        datos_pro += f"""
FORECAST DE VENTAS:
- Pesimista: ${_n(esc.get('pesimista',{}).get('promedio_mensual',0)):,}/mes (${_n(esc.get('pesimista',{}).get('total_anual',0)):,} año 1)
- Base: ${_n(esc.get('base',{}).get('promedio_mensual',0)):,}/mes (${_n(esc.get('base',{}).get('total_anual',0)):,} año 1)
- Optimista: ${_n(esc.get('optimista',{}).get('promedio_mensual',0)):,}/mes (${_n(esc.get('optimista',{}).get('total_anual',0)):,} año 1)"""
...
- Ticket referencial zona: ${_n(ticket_data['ticket_competencia']):,} MXN
...
- Ticket promedio zona: ${_n(ticket_data.get('ticket_competencia',0)):,} MXN
- Nivel de precio zona: {ticket_data.get('rango_zona','N/A')}
- Competidores con precio registrado: {ticket_data.get('n_con_precio',0)}"""
        if tier_key == "premium":
            datos_pro += f"""
- Ticket recomendado para el negocio: ${_n(ticket_data.get('ticket_recomendado',0)):,} MXN
...
- Utilidad mensual estimada: ${_n(roi_data.get('utilidad_mensual_est',0)):,}
