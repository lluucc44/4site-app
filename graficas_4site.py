...
Usa matplotlib con estilo 4SITE (negro #20364F + dorado #2A6FDB).
"""
...
C_BLUE   = "#2A6FDB"   # dorado (reemplaza azul)
C_CYAN   = "#20364F"   # negro (reemplaza cyan)
C_GREEN  = "#1F9D57"
C_YELLOW = "#2A6FDB"   # dorado
C_RED    = "#E0524A"
C_GRAY   = "#6B6B6B"
C_BG     = "#F5F5F5"
C_DARK   = "#20364F"
...
def grafica_edad_genero(demografia: dict, tipo_negocio_nombre: str = "") -> BytesIO:
    ...
    Colores 4SITE: dorado #2A6FDB para clientes potenciales, gris #6B6B6B para otros.
    """
    dem = demografia or {}
...
def grafica_dashboard_premium(score, desglose, mercado_data, forecast_data,
    ...
    for i, (label, valor) in enumerate(dem_items):
        ...
        ax_dem.add_patch(mpatches.FancyBboxPatch(
            ...
            boxstyle="round,pad=0.02",
            facecolor='#F5F8FF', edgecolor='#2A6FDB22', linewidth=0.5))
